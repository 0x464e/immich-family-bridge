...
- Member API keys with `user.read`, `asset.read`, `assetFile.read`, `album.create`, `album.read`, `album.update`, `albumAsset.create`, `albumAsset.delete`, `job.create`, `stack.read`, `stack.create`, and `stack.delete`. Add `asset.delete` when `remove_unshared_replicas` is enabled. The administrator key needs `library.read`, `library.update`, and `job.create`; the last scope starts Immich's sidecar discovery job when a recipient XMP appears after its media was already imported. These scopes were validated on Immich 3.2.2.
...
`FAMILYBRIDGE_DRY_RUN=true` is the default when the environment variable is omitted. The supplied Compose file also mounts the media tree read-only by default. The service stays running: it previews once at startup and then at `discovery_interval` (five minutes by default), logging each proposed action with the album, member, and asset IDs, followed by an action count. Watch it with `docker compose logs -f family-bridge` or the container logs in your deployment UI. On the first dry-run startup, the bridge records its catch-all album in SQLite and logs a proposed album creation for every member whose matching album does not already exist in Immich. For an asset in an attached album, it logs “new source asset found” and “would share asset with member through hardlink and import.” The same actions can appear on later previews because dry-run deliberately does not complete them.

In active mode, a complete album discovery runs on startup and at `discovery_interval` (default five minutes). Between discoveries, bounded work passes run at `poll_interval` (default 30 seconds) to finish known imports, album writes, stacks, and cleanup without searching all albums again. Both intervals start after the prior pass finishes, so a long pass cannot trigger an immediate second pass. `POST /api/reconcile` forces a complete discovery. Source edits made between discoveries may take up to the next full pass to be noticed. Set `discovery_interval` longer during sustained uploads if delayed mirroring is acceptable; this is a periodic schedule, not an upload-quiet detector.
