...
6. Removing an asset from a registered album removes the translated asset from every member's matching album. If another registered album still references it, its recipient replica remains. After the last reference is removed, the default behavior retains recipient assets as `pending_removal`. Set `remove_unshared_replicas: true` to enable verified cleanup: the bridge waits one polling interval, rechecks the reference count and recipient identity, asks Immich to force-delete only the recipient asset, then safely unlinks any bridge-managed media and XMP paths Immich left behind. The origin asset remains in its owner's library.
